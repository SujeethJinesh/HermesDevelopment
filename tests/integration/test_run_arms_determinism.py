...
class TestRunArmsDeterminism(unittest.TestCase):
    ...
    def test_identical_runs_produce_identical_metrics(self):
        ...
        result1 = subprocess.run(
            [
                sys.executable, "-m", "eval.run_arms",
                "--arm", arm,
                "--seed", str(seed),
        ...
        for m1, m2 in zip(metrics1, metrics2):
            ...
            for key in ["start_time", "end_time", "duration", "sandbox_cleanup_ms", "sandbox_setup_ms"]:
                m1.pop(key, None)
                m2.pop(key, None)
        ...
        time_cols = [c for c in df1.columns if "time" in c.lower() or "duration" in c or "cleanup" in c or "setup" in c]
        ...
        print(f"✓ Two identical runs produced identical metrics")
    ...
    def test_different_seeds_produce_different_metrics(self):
        ...
        print(f"✓ Different seeds produced different metrics")
    ...
    def test_config_override_rejection(self):
        ...
        result = subprocess.run(
            [
                ...
                "--arm", "A",
                "--seed", "123",
                "--gen_cfg", "custom_config.yaml",  # Non-canonical config
                "--hermetic", "on",
                "--toy", "1"
            ],
            capture_output=True,
        ...
        print(f"✓ Config override correctly rejected")
...
""")
        ...
        print(f"✓ Hermetic mode enabled successfully")
    ...
    def test_metrics_schema_compliance(self):
        ...
        print(f"✓ Metrics comply with schema")
    ...
    def test_parquet_deterministic_hash(self):
        ...
        df1_clean = df1.drop(columns=[c for c in df1.columns if "time" in c.lower()], errors="ignore")
        content1 = df1_clean.to_json(orient="records", sort_keys=True)
        ...
        df2_clean = df2.drop(columns=[c for c in df2.columns if "time" in c.lower()], errors="ignore")
        content2 = df2_clean.to_json(orient="records", sort_keys=True)
