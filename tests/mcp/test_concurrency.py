    ...
    def test_no_torn_writes(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            def make_data(worker_id: int, size: int = 10000) -> bytes:
                ...
                return b"START|" + f"{worker_id}|".encode() + payload + f"|{worker_id}|".encode() + MARKER
                        ...
                        if data == initial_data:
                            ...
                            if len(parts) > 1:
                                ...
                                if numbers and len(set(numbers)) > 1:
                                    inconsistencies.append(f"Reader {reader_id}: Mixed updates {set(numbers)}")
                        else:
            ...
            def stat_checker(checker_id: int):
                ...
                for _ in range(40):
                    ...
                    if meta:
                        ...
                        data = client.resolve(ref)
                        if data is None:
                            inconsistencies.append(f"Checker {checker_id}: stat exists but resolve failed")
