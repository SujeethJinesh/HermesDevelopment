...
class TestTransportRTT:
            ...
            try:
                ...
                for i in range(num_calls):
                    ...
                    _, rtt_ms = transport.call_agent(
                        f"bench-{i}", "planner",
                        req.SerializeToString(),
                        "application/x-protobuf", f"trace-{i}"
                    )
    ...
    def test_json_vs_protobuf_rtt(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            for arm in ["A", "C"]:
                ...
                try:
                    ...
                    for i in range(500):
                        ...
                        _, rtt_ms = transport.call_agent(
                            f"bench-{i}", "planner",
                            payload, content_type, f"trace-{i}"
                        )
                        rtts.append(rtt_ms)
            ...
            for arm in ["A", "C"]:
                r = results[arm]
                print(f"  Arm {arm}: mean={r['mean']:.3f}ms, p50={r['p50']:.3f}ms, p95={r['p95']:.3f}ms")
