...
class TestHermeticRun:
    ...
    def test_network_blocked_in_venv_py(self, tmp_path):
        ...
        # Create hermetic run
        run = HermeticRun(task_id="test_network", seed=42, hermetic=True)
...
try:
    ...
    err_str = str(e).lower()
    if "hermetic" in err_str or "blocked" in err_str or "permission" in err_str:
...
"""
    ...
    def test_worktree_detached_and_clean(self):
        ...
        run = HermeticRun(task_id="test_worktree", seed=123, base_sha=current_sha, hermetic=True)
        ...
        with run():
            ...
            ), f"Worktree SHA mismatch: {worktree_sha} != {current_sha}"
    ...
    def test_manifest_fields_and_stable_hash(self):
        ...
        for i in range(2):
            run = HermeticRun(task_id="test_manifest", seed=999, hermetic=True)
        ...
        assert (
            manifests[0]["stable_hash"] == manifests[1]["stable_hash"]
        ), "Stable hash not deterministic for same seed"

        # Check that run_ids are different
        assert manifests[0]["run_id"] != manifests[1]["run_id"], "Run IDs should be unique"
    ...
    def test_cleanup_no_residue(self):
        ...
        # Create hermetic run
        run = HermeticRun(task_id=task_id, seed=456, hermetic=True)
