...
### Hermetic Evaluation Results (2025-08-26)

✅ **Infrastructure complete**: All hermetic components in PR #6  
✅ **Dataset loads offline**: 20 instances from SWE-bench Lite slice20  
✅ **Both arms executed**: C and PM completed evaluation  
❌ **FAILED: Bytes reduction**: PM (4145.2) > C (3077.8) - opposite of requirement  
❌ **FAILED: Pass@1 parity**: PM 0% vs C 77.5% (delta 77.5pp > 2pp)  

### Root Cause Analysis

The PM arm fails acceptance because:

1. **MCP references not resolved**: When PM stores patches as MCP anchors (e.g., "mcp://pm/abc123"), the test runner receives the reference string instead of the actual patch content. This causes "corrupt patch" errors for all tests.

2. **Higher bytes paradox**: Even though PM uses MCP anchors, it shows HIGHER bytes (4145 vs 3078) because:
   - Protobuf overhead is larger than expected
   - MCP references are being counted as part of the payload
   - The patch corruption causes longer error messages

3. **Implementation gap**: The `agents/pm_arm.py` correctly creates MCP anchors but the test pipeline doesn't resolve them before applying patches.

### Actual Metrics from Hermetic Run

```json
{
  "arm_C": {
    "bytes_per_solve": 3077.85,
    "pass_at_1": 0.775,
    "message_path_ms_p95": 0.001,
    "tasks_passed": "15/20"
  },
  "arm_PM": {
    "bytes_per_solve": 4145.2,
    "pass_at_1": 0.0,
    "message_path_ms_p95": 4562,
    "tasks_passed": "0/20"
  }
}
```

### Files Successfully Added to PR #6

✅ **env/hermetic_repos.py** - Hermetic repo manager  
✅ **prepare_hermetic_run.sh** - Evaluation workflow script  
✅ **.github/workflows/ban_artifacts.yml** - CI guard against artifacts  
✅ **scripts/check_acceptance_final.py** - Acceptance criteria checker  

### Next Steps Required

1. **Fix MCP resolution**: Modify test pipeline to resolve MCP references before applying patches
2. **Reduce protobuf overhead**: Optimize protobuf serialization
3. **Rerun evaluation**: After fixes, demonstrate PM < C bytes
4. **Update PR**: Push fixes and updated metrics
